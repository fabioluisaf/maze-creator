...
async function search(start, end, openSet, heuristicFunction, delay) {
  ...
  while (openSet.size() > 0) {
    ...
    let min = openSet.pop();
    // marking it as visitedByPathFinder, so that it don't get re-added to the set
    min.visitedByPathFinder = true;
 ...
 * walls, were not visitedByPathFinder and whose gcost is greater than the current node's + 1
 * 
...
function filteredNeighbors(node) {
  ...
  for (let neighbor of node.neighbors) {
    if (!neighbor.isWall && !neighbor.visitedByPathFinder && neighbor.gcost >= node.gcost + 1) {
...
/**
 * Updates a node gcost, hcost and predecessor if necessary.
 * 
 ...
 * 
 * @param {*} newPredecessor         The node that will possibly be the new predecessor.
 * 
 * @param {*} end               The end node of the search, used to calculate the hcost.
 */
function updateNode(openSet, heuristicFunction, node, newPredecessor, end) {
  if (node.hcost === Infinity) {
    node.hcost = heuristicFunction(node, end);
  }

  openSet.updatePriority(node, newPredecessor);
}
